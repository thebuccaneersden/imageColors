...
extension UIImage {
    ...
    public func getColors(completionBlock:(imageColors:UIImageColors) -> Void) {
        var result = UIImageColors()
        
        let ratio = self.size.width/self.size.height
        let r_width: CGFloat = 250

        let scaleDownSize = CGSizeMake(r_width, r_width/ratio)
        ...
        let sortedColorComparator: NSComparator = { (main, other) -> NSComparisonResult in
            let m = main as! EnumerateColor, o = other as! EnumerateColor
            if m.count < o.count {
        ...
        while let kolor = enumerator.nextObject() as? UIColor {
            ...
            if randomColorsThreshold < colorCount  {
                sortedColors.addObject(EnumerateColor(color: kolor, count: colorCount))
            }
        }
        sortedColors.sortUsingComparator(sortedColorComparator)
        
        var proposedEdgeColor: EnumerateColor
        if 0 < sortedColors.count {
            proposedEdgeColor = sortedColors.objectAtIndex(0) as! EnumerateColor
        } else {
            proposedEdgeColor = EnumerateColor(color: blackColor, count: 1)
        }
        
        if proposedEdgeColor.color.isBlackOrWhite && 0 < sortedColors.count {
            for i in 1..<sortedColors.count {
                let nextProposedEdgeColor = sortedColors.objectAtIndex(i) as! EnumerateColor
        ...
        for curContainer in sortedColors {
            let kolor = (curContainer as! EnumerateColor).color
        ...
        if result.allColors == nil {
            result.allColors = sortedColors
        }
        ...
        completionBlock(imageColors: result)
